...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  ...
  let myProd = multiply(a, multiply(b, c)[0])[0];
  let sumStr = `${a} and ${b} and ${c} sum to ${mySum}.`;
  let prodStr = `The product of ${a} and ${b} and ${c} is ${myProd}.`;
  return [mySum, myProd, sumStr, prodStr];
}
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  let myArr = `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${mySum} is their sum.`;
  return [mySum, myArr];
}
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  let myProd = 1;
  let myArr = 'The numbers ';
  for (let i = 0; i < dynamicArray.length; i++) {
    myProd = multiply(myProd, dynamicArray[i])[0];
    myArr = myArr.concat(dynamicArray[i]);
    if (i < (dynamicArray.length - 1)) {
      myArr = myArr.concat(',');
    }
  }
  myArr += ` have a product of ${myProd}.`;
  return [myProd, myArr];
}
